"""
# This code is based on https://github.com/willisma/SiT
"""
...
class Transport:
    ...
    def training_losses(
        ...
        # print ("x1", x1.shape, x1.mean(), x1.std(), x1.min(), x1.max())
        # print ("model_output", model_output.shape, model_output.mean(), model_output.std(), model_output.min(), model_output.max())
        ...
        if self.model_type == ModelType.VELOCITY:
            ...
            if use_contrastive_loss:
                ut_tlide = th.roll(ut, shifts=1, dims=0)
                terms["loss"] += (
                    -mean_flat(((model_output - ut_tlide) ** 2), dim=dim) * 0.05
                )
            if ae is not None:
