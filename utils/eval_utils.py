...
def evaluation_ae(
    ...
    for batch in tqdm(val_loader):
        ...
        (et, em), (_, _) = eval_wrapper.get_co_embeddings(
            word_embeddings, pos_one_hots, sent_len, caption, bgt, m_length - 1
        )
        ...
        (et_pred, em_pred), (_, _) = eval_wrapper.get_co_embeddings(
            ...
            sent_len,
            caption,
            torch.from_numpy(bpred).to(device),
            m_length - 1,
        )
        for i in range(bs):
            gtt = recover_from_ric(
                torch.from_numpy(gt[i, : m_length[i]]).float(), 22
            ).reshape(-1, 22, 3)
            predd = recover_from_ric(
                torch.from_numpy(pred[i, : m_length[i]]).float(), 22
            ).reshape(-1, 22, 3)
    ...
    print(msg)
    if draw:
        metric_step = ep if log_step is None else log_step
        writer.log({f"Test/FID": fid}, step=metric_step)
        writer.log({f"Test/Diversity": diversity}, step=metric_step)
        writer.log({f"Test/top1": R_precision[0]}, step=metric_step)
        writer.log({f"Test/top2": R_precision[1]}, step=metric_step)
        writer.log({f"Test/top3": R_precision[2]}, step=metric_step)
        writer.log({f"Test/matching_score": matching_score_pred}, step=metric_step)
        writer.log({f"Test/MPJPE": mpjpe}, step=metric_step)
...
def evaluation_dit(
    ...
    for i, batch in enumerate(tqdm(val_loader)):
        ...
        if i < num_mm_batch:
            ...
            for _ in tqdm(range(30)):
                pred_latents = ema_model.generate(
                    clip_text,
                    m_length // 4,
                    cond_scale,
                )

                pred_latents = rearrange(pred_latents, "b l 1 d -> b l d")
                if after_mean is not None and after_std is not None:
                    pred_latents = pred_latents * after_std + after_mean
                pred_latents = rearrange(pred_latents, f"b l d -> {ae.encode_dim}")
                with torch.no_grad():
                    pred_motions = ae.decode(pred_latents)

                pred_motions = val_loader.dataset.inv_transform(
                    pred_motions.detach().cpu().numpy(),
                    mean=val_loader.dataset.mean[: pred_motions.shape[-1]],
                    std=val_loader.dataset.std[: pred_motions.shape[-1]],
                )
                pred_motions_list = []
                for j in range(bs):
                    feature_dim = eval_mean.shape[0]
                    pred_motions_list.append(pred_motions[j, :, 0, :feature_dim])

                pred_motions = np.stack(pred_motions_list, axis=0)
                pred_motions = val_loader.dataset.transform(
                    pred_motions, eval_mean, eval_std
                )
                (et_pred, em_pred), (
                    et_pred_clip,
                    em_pred_clip,
        ...
        for j in range(bs):
            feature_dim = eval_mean.shape[0]
            poses.append(pose[j, :, 0, :feature_dim])
    ...
    fid = calculate_frechet_distance(gt_mu, gt_cov, mu, cov)
    fid_real = calculate_frechet_distance(gt_mu, gt_cov, gt_mu, gt_cov)

    msg = (
        f"--> \t Eva. Ep/Re {ep} :, FID. {fid:.4f}, Diversity Real. {diversity_real:.4f}, Diversity."
