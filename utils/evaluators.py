...
def build_evaluators(
    ...
    movement_enc = MovementConvEncoder(
        dim_pose, dim_movement_enc_hidden, dim_movement_latent
    )
    ...
    contrast_model = MotionCLIP(67)

    if feature_dim == 67:
        checkpoint = torch.load(
            os.path.join(
                checkpoints_dir, dataset_name, "text_mot_match", "model", "finest.tar"
            ),
            map_location=device,
        )
...
class Evaluators(object):
    def __init__(self, dataset_name, device, feature_dim=None):
        if dataset_name == "t2m":
            dim_pose = feature_dim if feature_dim is not None else 259
        elif dataset_name == "kit":
            dim_pose = 64  # Keep original for kit dataset
        else:
            raise KeyError("Dataset not Recognized!!!")

        # Store the feature dimension for use in encoding methods
        self.feature_dim = dim_pose
    ...
    def get_co_embeddings(
        self, word_embs, pos_ohot, cap_lens, captions, motions, m_lens
    ):
        with torch.no_grad():
            ...
            """clip based"""
            # Always use first 67 dimensions for CLIP model (original features)
            clip_em = self.contrast_model.encode_motion(
                motions.clone()[..., :67], m_lens
            )
            ...
            # Use feature_dim to determine how many dimensions to use
            if self.feature_dim == 67:
                # Use original 67 dimensions
                movements = self.movement_encoder(motions[..., :67]).detach()
            else:
                # Use current behavior: all except last 4 dimensions
...
class MotionEncoder(nn.Module):
    def __init__(
        self, in_dim, latent_dim, ff_size, num_layers, num_heads, dropout, activation
    ):
        super().__init__()
        ...
        self.sequence_pos_encoder = PositionalEncodingCLIP(
